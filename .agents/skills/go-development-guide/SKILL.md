...
- You need scoped permission patterns (satker/unit restrictions)
- You need a pre-push checklist before committing code

> ⚠️ **For coding rules** (naming conventions, anti-patterns, helpers reference, GenericRepository methods), use the **go-coding-rules** skill. This skill focuses on the implementation flow and workflow.
...
- ✅ **Write with result reload** → `s.repo.TxManager.WithinTransactionWithResult()` (for complex operations like role assignment)
...
- ✅ **Bind JSON** → `c.BindJSON()` + `h.Validate.Struct()` with validation
