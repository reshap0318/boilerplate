...
// RoleAttachPermissions handles POST /api/roles/:id/permissions
func (h *Handlers) RoleAttachPermissions(c *gin.Context) {
	id, err := strconv.ParseUint(c.Param("id"), 10, 64)
	if err != nil {
		helpers.BadRequest(c, "Invalid role ID")
		return
	...
	var req dtos.AttachPermissionsRequest
	if err := c.ShouldBindJSON(&req); err != nil {
		helpers.BadRequest(c, err.Error())
		return
	}

	err = h.svcs.AttachPermissions(c.Request.Context(), uint(id), req)
	if err != nil {
		helpers.InternalServerError(c, "Failed to attach permissions")
		return
	}

	helpers.OK(c, "Permissions attached successfully", nil)
}

// RoleDetachPermissions handles DELETE /api/roles/:id/permissions
func (h *Handlers) RoleDetachPermissions(c *gin.Context) {
	id, err := strconv.ParseUint(c.Param("id"), 10, 64)
	if err != nil {
		helpers.BadRequest(c, "Invalid role ID")
		return
	...
	err = h.svcs.DetachPermissions(c.Request.Context(), uint(id))
	if err != nil {
		helpers.InternalServerError(c, "Failed to detach permissions")
		return
	}

	helpers.OK(c, "Permissions detached successfully", nil)
}
